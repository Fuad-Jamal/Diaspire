...
const Dashboard = () => {
  ...
  if (fullName) {
    setMentorName(fullName);
    const parts = fullName.split(' ');
    const initials = parts[0]?.[0] + (parts[1]?.[0] || '');
    setMentorInitials(initials.toUpperCase());
  }

  if (bio) setMentorBio(bio);
  if (linkedin) setMentorLinkedIn(linkedin);
}, []);


  return (
    <div className="flex flex-col w-auto h-[60%] rounded-b-lg px-8 py-8 bg-white border-r fixed left-0">
      <div className="flex items-center mb-8 px-2">
        ...
        <div className="ml-2">
          ...
          {mentorBio && (
  <p className="text-sm text-gray-600 mt-1 italic">{mentorBio}</p>
)}
{mentorLinkedIn && (
  <a
    href={mentorLinkedIn}
  ...
  >
    View LinkedIn Profile
      ...
      <nav className="flex-1 space-y-2 pb-12">
        {navItems.map((item) => (
          <div
            key={item.name}
            className={`
