...
export default function Testimonies() {
  ...
  //Rule to set live testimonies to appear on single page
    const liveTestimonies = testimonies.slice(livePage, livePage+testimoniesPerPage).map((testimony,index)=>{
      return (
        <div className="bg-blue-400 mx-auto rounded-2xl relative z-0 hover:z-10 hover:scale-105 transition-transform duration-300 hover:shadow-xl">
          <div className="bg-gray-400 m-5 p-2 w-80 h-96 rounded-b-full relative z-0 hover:z-10 hover:scale-105 transition-transform duration-300 hover:shadow-xl">
...
const changePage = ({selected})=>{
  ...
  return (
    <div className="bg-[url(/src/assets/testimonyBG.png)] bg-no-repeat py-4 ">
      <h1 className="text-2xl font-bold text-center m-4 text-white">What Our Community Say About Us</h1>
      <ReactPaginate
  previousLabel={"<"}
  nextLabel={">"}
  pageCount={pageCount}
